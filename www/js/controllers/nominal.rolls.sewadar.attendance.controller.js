      ...
      **/
      var NominalRollsSewadarAttendanceController = function($log, $scope, $state, $ionicHistory, $cordovaSQLite, $ionicPopover, $ionicModal, $filter, ionicDatePicker, $stateParams, $rootScope, $timeout, $cordovaToast, cfpLoadingBar, nominalRollsService, $cordovaPrinter, authService, $ionicPopup, $cordovaFile, profilePicService, $ionicActionSheet) {
            ...
            $scope.openPopoverLetterNumber = function($event) {
                  if ($scope.nominalRollsData.status != 'dispatched') {
                        $cordovaToast.show('Please dispach the nominal role first.', 'short', 'center');
                        return;
                  }
            ...
            var getContactPersonDetails = function () {
                  var query = "SELECT * FROM sewas WHERE id =" + $scope.nominalRollsData.sewa_id;
                  $cordovaSQLite.execute($rootScope.db, query).then(function (res) {
                        for (var i = 0; i < res.rows.length; i++) {
                              $scope.contact_person_name = res.rows.item(i).contact_person_name ? res.rows.item(i).contact_person_name : '__________';
                              $scope.contact_person_phone = res.rows.item(i).contact_person_mob_number ? res.rows.item(i).contact_person_mob_number : '__________';
                        }
                  });
            }
            setup();
